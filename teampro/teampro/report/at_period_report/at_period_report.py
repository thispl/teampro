...
def get_data(filters):
	...
	if filters.completed_by:
		...
		if emp:
			...
			for j in timesheet:
				task = frappe.get_value("Task",{'name':j.task},['project','subject','priority','status','revisions','expected_time','custom_code_reviewer_comment','custom_allocated_to'])
				...
				if task:	
					...
					for at in actual_time:
						...
						if task[5] and task[5] > 0:
							...
							etper = 0
						row = [emp,emp_short_code,task[7],task[0],j.task,task[1],task[2],task[3],task[4],task[5],round(atime,2),round(j.hours,2),round(ettime,2),round(etper,2),task[6]]
						data.append(row)
				if issue:
					...
					for at in actual_time:
						...
						ettime=0.5-atime
						etper=(atime/0.5)*100
						row = [emp,emp_short_code,'NA',issue[0],j.issue,issue[1],issue[2],issue[3],'NA',0.5,round(atime,2),round(j.hours,2),round(ettime,2),round(etper,2),'']
						data.append(row)
				if meeting:
					actual_time = frappe.db.sql(""" select sum(`tabTimesheet Detail`.hours) as hours from `tabTimesheet`	left join `tabTimesheet Detail` on `tabTimesheet`.name = `tabTimesheet Detail`.parent where  `tabTimesheet`.docstatus = 1 and `tabTimesheet Detail`.activity_type != "Manual Testing" and `tabTimesheet Detail`.custom_meeting = '%s' """%(j.meeting),as_dict = 1) or ''
					for at in actual_time:
						atime = at.hours or 0
						row = [emp,emp_short_code,'NA',meeting[0],j.meeting,meeting[1],'',meeting[2],'NA','',round(atime,2),round(j.hours,2),'','','']
						data.append(row)
				...
				if task:
					...
					for at in actual_time:
						...
						if task[5] and task[5] > 0:
							...
							etper = 0
						row = [e.name or '',e.short_code or '',task[7] or '',task[0] or '',j.task or '',task[1] or '',task[2] or '',task[3] or '',task[4] or '',task[5] or '',round(atime,2) or '',round(j.hours,2) or '',round(ettime,2) or '',round(etper,2) or '',task[6] or '']
						data.append(row)
				if issue:
					# actual_time = frappe.db.sql(""" select sum(`tabTimesheet Detail`.hours) as hours from `tabTimesheet` left join `tabTimesheet Detail` on `tabTimesheet`.name = `tabTimesheet Detail`.parent where  `tabTimesheet`.docstatus = 1 and `tabTimesheet Detail`.activity_type != "Manual Testing" and `tabTimesheet Detail`.custom_issue = '%s' """%(j.issue),as_dict = 1) or ''
					...
					for at in actual_time:
						...
						ettime=0.5-atime
						etper=(atime/0.5)*100
						row = [e.name,e.short_code,'NA',issue[0],j.issue,issue[1],issue[2],issue[3],'NA',0.5,round(atime,2),round(j.hours,2),round(ettime,2 ),round(etper,2),'']
						data.append(row)
				if meeting:
					actual_time = frappe.db.sql(""" select sum(`tabTimesheet Detail`.hours) as hours from `tabTimesheet`	left join `tabTimesheet Detail` on `tabTimesheet`.name = `tabTimesheet Detail`.parent where  `tabTimesheet`.docstatus = 1 and `tabTimesheet Detail`.activity_type != "Manual Testing" and `tabTimesheet Detail`.custom_meeting = '%s' """%(j.meeting),as_dict = 1) or ''
					for at in actual_time:
						atime = at.hours or 0
						row = [e.name,e.short_code,'NA',meeting[0],j.meeting,meeting[1],'',meeting[2],'NA','',round(atime,2),round(j.hours,2),'','','']
						data.append(row)
