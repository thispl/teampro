...
from frappe.utils import getdate, get_timespan_date_range,flt
import json
...
def get_sc_value(month=None,year =None):
    try :
        ...
        add = 0
        sr = 0
        total_dec = frappe.db.sql("""select sum(total_sc_company_currency) as total from `tabSales Invoice` where month(posting_date)= '%s' and status in ("Paid","Overdue","Unpaid") and year(posting_date) = '%s' """ %(month,year),as_dict=True)
        ...
        for sc in short_code:
            ...
            if emp.short_code =='API':
                acheieved_till_date =frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE services in ("IT-SW","IT-IS") AND status in ("Paid","Overdue","Unpaid") AND posting_date between '%s' and '%s' """ %(april_month,last_day_of_prev_month),as_dict=True)[0]

            if emp.short_code == 'SBMK':
                acheieved_till_date =frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE services ="TFP" AND status in ("Paid","Overdue","Unpaid") AND posting_date between '%s' and '%s' """ %(april_month,last_day_of_prev_month),as_dict=True)[0]
              
            if emp.short_code == 'SP':
                acheieved_till_date =frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE services ="TGT" AND status in ("Paid","Overdue","Unpaid") AND posting_date between '%s' and '%s' """ %(april_month,last_day_of_prev_month),as_dict=True)[0]
               
            if emp.short_code == 'AS':
                acheieved_till_date =frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE services not in ("TFP","IT-SW","TGT","IT-IS") AND status in ("Paid","Overdue","Unpaid") AND posting_date between '%s' and '%s' """ %(april_month,last_day_of_prev_month),as_dict=True)[0]
    ...
    for sc in short_code:
        ...
        try:
            if emp.based_on_value == "Role Based":
                ...
                acc_si  = frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE account_manager = '%s' AND delivery_manager != '%s'  AND month(posting_date) ='%s' AND status in ("Paid","Overdue","Unpaid") AND YEAR(posting_date) = '%s'""" % (
                    emp.prefered_email,emp.prefered_email,month,year),as_dict=True)

                dev_si = frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE delivery_manager = '%s' AND account_manager != '%s' AND month(posting_date) ='%s'  AND status in ("Paid","Overdue","Unpaid") AND YEAR(posting_date) = '%s'""" % (
                    emp.prefered_email,emp.prefered_email,month,year),as_dict=True)
               
                
                both_si = frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE delivery_manager = '%s' AND account_manager = '%s' AND month(posting_date) ='%s' AND status in ("Paid","Overdue","Unpaid") AND YEAR(posting_date) = '%s'""" % (
                    emp.prefered_email,emp.prefered_email,month,year),as_dict=True)
                ...
                if service_list:
                    service_si  = frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE services IN (%s) AND month(posting_date) = '%s' AND status in ("Paid","Overdue","Unpaid") AND YEAR(posting_date) = '%s'""" % (
...
def get_sc_value_bt(month=None,year =None):
    ...
    for sc in short_code:
        ...
        if emp.short_code == 'SP':
            acheieved_till_date =frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE services ="TGT" AND status in ("Paid","Overdue","Unpaid") AND posting_date between '%s' and '%s' """ %(april_month,last_day_of_prev_month),as_dict=True)[0]
            frappe.errprint(acheieved_till_date)          
        if emp.short_code == 'AS':
            acheieved_till_date =frappe.db.sql("""select sum(total_sc_company_currency) as total from`tabSales Invoice` WHERE services not in ("TFP","IT-SW","TGT","IT-IS") AND status in ("Paid","Overdue","Unpaid") AND posting_date between '%s' and '%s' """ %(april_month,last_day_of_prev_month),as_dict=True)[0]
