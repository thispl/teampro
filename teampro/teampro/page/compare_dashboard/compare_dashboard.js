frappe.pages['compare-dashboard'].on_page_load = function (wrapper) {
...
frappe.pages['compare-dashboard'].on_page_show = function (wrapper) {
...
const FA_TRANSLATIONS = {
	...
	sec_performance_summary: { ar: 'ملخص الأداء', en: 'Performance Summary' },
	sec_performance_summary_desc: { ar: 'حجز الطلبات، المبيعات، والذمم الدائنة شهرياً', en: 'Order Booking, Turnover, and Payables monthly' },
	...
	chart_turnover: { ar: 'المبيعات', en: 'Turnover' },
	chart_order_booking: { ar: 'حجز الطلبات', en: 'Order Booking' },
	chart_payables: { ar: 'الذمم الدائنة', en: 'Payables' },
...
class FinancialAuditDashboard {
	constructor(page) {
		...
		this.section_registry = [
			...
			{ key: 'performance-summary-chart', label_key: 'sec_performance_summary' },
	...
	// 	this.$performance_summary_chart = this.page.main.find('.performance-summary-chart-chart');
	// 	this.$performance_summary_stats = this.page.main.find('.performance-summary-stats');
	...
	setup_page() {
		...
		if (!document.getElementById('fa-global-style')) {
			$(`<style id="fa-global-style">
				.fa-page-header{ position:relative; padding:10px; text-align:center; }
				.fa-page-header .fa-page-title{ font-weight:bold; margin:0; }
				...
				.fa-tab-bar .fa-tab i{
					font-size:16px;
					line-height:1;
					/* Font Awesome icons are font glyphs — bolder font-weight
					   makes the icon stroke thicker for stronger visual presence. */
					font-weight:900;
				}
				/* Per-tab colors — applied to the whole tab (icon + text).
				   Only the active tab shows its assigned color; all inactive
				   tabs are black (icon + text). The active tab keeps its
				   matching colored underline. */
				.fa-tab-bar .fa-tab{ color:#000; }
				.fa-tab-bar .fa-tab:not(.fa-tab-active):hover{ color:#000; opacity:0.7; }

				/* Active state: full assigned color, bold, matching underline */
				.fa-tab-bar .fa-tab.fa-tab-active{
					background:transparent;
					font-weight:700;
					opacity:1;
				}
				.fa-tab-bar .fa-tab.fa-tab-active[data-tab="graph"]      { color:#10b981; border-bottom-color:#10b981; }
				.fa-tab-bar .fa-tab.fa-tab-active[data-tab="data"]       { color:#2563eb; border-bottom-color:#2563eb; }
				.fa-tab-bar .fa-tab.fa-tab-active[data-tab="approvals"]  { color:#059669; border-bottom-color:#059669; }
				.fa-tab-bar .fa-tab.fa-tab-active[data-tab="transaction"]{ color:#f97316; border-bottom-color:#f97316; }
				.fa-tab-bar .fa-tab.fa-tab-active[data-tab="receivable"] { color:#ca8a04; border-bottom-color:#ca8a04; }
				.fa-tab-bar .fa-tab.fa-tab-active[data-tab="payable"]    { color:#7c3aed; border-bottom-color:#7c3aed; }
				.fa-finance-content{ display:block; }
				.fa-finance-content.fa-finance-hidden{ display:none; }
				.fa-tab-panel{ display:none; }
				.fa-tab-panel.fa-tab-panel-active{ display:block; }
				.fa-tab-placeholder{
					padding:40px 20px;
					text-align:center;
					color:#9ca3af;
				...
				/* Iframe wrapper for Approvals / Transaction tabs — removed, now inline */
		...
		this.page.main.html(`
			<div class="financial-audit-page ${this.is_rtl ? '' : 'ltr-mode'}" dir="${dir}">
				<div class="fa-tab-bar" id="fa-tab-bar">
					<button type="button" class="fa-tab fa-tab-active" data-tab="graph"><i class="fa fa-line-chart"></i> ${this.lang === 'ar' ? 'الرسم البياني' : 'Graph'}</button>
					<button type="button" class="fa-tab" data-tab="data"><i class="fa fa-table"></i> ${this.lang === 'ar' ? 'البيانات' : 'Data'}</button>
					<button type="button" class="fa-tab" data-tab="approvals"><i class="fa fa-check-square-o"></i> ${this.lang === 'ar' ? 'الموافقات' : 'Approvals'}</button>
					<button type="button" class="fa-tab" data-tab="transaction"><i class="fa fa-exchange"></i> ${this.lang === 'ar' ? 'الحركات' : 'Transaction'}</button>
					<button type="button" class="fa-tab" data-tab="receivable"><i class="fa fa-money"></i> ${this.lang === 'ar' ? 'المستحقات' : 'Receivable'}</button>
					<button type="button" class="fa-tab" data-tab="payable"><i class="fa fa-credit-card"></i> ${this.lang === 'ar' ? 'المدفوعات' : 'Payable'}</button>
				</div>

				<div class="fa-finance-content">
					<div class="fa-page-header">
						<h2 class="fa-page-title">${this.lang === 'ar' ? 'المالية والحسابات' : 'Finance & Accounts'}</h2>
						<div id="fa-current-datetime" class="fa-current-datetime"></div>
					</div>
					<div class="filters-section"></div>
				...
				<div class="fa-tab-panel fa-tab-panel-active" data-tab-panel="graph">
			...
			this.t('sec_balance_sheet_desc'))}
				...
				<!-- ✅ DATA VIEW WRAPPER — new finance-new.js content -->
				<div class="fa-tab-panel" data-tab-panel="data">
				<div class="data-view-wrapper" style="display:none;">
					<div id="finance-new-mount"></div>
				</div>
				</div>
				<!-- /fa-tab-panel[data] -->

				<!-- Approvals tab — mount point (content built by load_approvals_view) -->
				<div class="fa-tab-panel" data-tab-panel="approvals">
					<div id="approvals-mount"></div>
				</div>

				<!-- Transaction tab — mount point (content built by load_transaction_view) -->
				<div class="fa-tab-panel" data-tab-panel="transaction">
					<div id="transaction-mount"></div>
				</div>

				<!-- Receivable tab — mount point (content built by load_rec_payable_view) -->
				<div class="fa-tab-panel" data-tab-panel="receivable">
					<div id="receivable-mount"></div>
				</div>

				<!-- Payable tab — mount point (content built by load_rec_payable_view) -->
				<div class="fa-tab-panel" data-tab-panel="payable">
					<div id="payable-mount"></div>
				</div>

			</div>
		`);

		// Cache references — same as before
		this.$filters = this.page.main.find('.filters-section');
		this.$kpi = this.page.main.find('.kpi-cards');
		this.$monthly_chart = this.page.main.find('.monthly-chart-chart');
		this.$monthly_stats = this.page.main.find('.monthly-chart-stats');
		...
		// ✅ Tab bar switching
		const self = this;
		this.page.main.on('click', '.fa-tab-bar .fa-tab', function () {
			self.switch_view($(this).data('tab'));
		});
		...
		// ✅ Default view = graph
		this.current_view = 'graph';
		this.data_view_loaded = false;
		this.update_view_btn_state();
		// ✅ Live clock
		const $fa_dt = this.page.main.find('#fa-current-datetime');
		function update_fa_datetime() {
			const now = new Date();
			const dateStr = now.toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' });
			const timeStr = now.toLocaleTimeString();
			$fa_dt.html(`${dateStr} | ${timeStr}`);
		}
		update_fa_datetime();
		setInterval(update_fa_datetime, 1000);

	}

	// ✅ Toggle function — drives the tab-based interface
	switch_view(view) {
		this.current_view = view;

		// Hide all tab panels, show the selected one
		this.page.main.find('.fa-tab-panel').removeClass('fa-tab-panel-active');
		this.page.main.find(`.fa-tab-panel[data-tab-panel="${view}"]`).addClass('fa-tab-panel-active');

		// Finance content (heading, filters, AI section, KPI cards) is only
		// relevant for the Data and Graph tabs. Hide it for the others.
		const is_finance_tab = (view === 'data' || view === 'graph');
		this.page.main.find('.fa-finance-content').toggleClass('fa-finance-hidden', !is_finance_tab);

		// Lazy-load the Data view on first activation
		if (view === 'data' && !this.data_view_loaded) {
			// The inner data-view-wrapper carries an inline display:none — clear it now
			this.page.main.find('.data-view-wrapper').css('display', '');
			this.load_data_view();
			this.data_view_loaded = true;
		}

		// Resize any echarts instances when returning to the graph tab
		if (view === 'graph') {
			Object.values(this.echarts_instances || {}).forEach(chart => {
				if (chart && !chart.isDisposed()) chart.resize();
			});
		...
		// Load approvals.js content directly for the Approvals / Transaction tabs
		if (view === 'approvals' || view === 'transaction') {
			this.load_approvals_iframe(view);
		}

		// Load the Receivables & Payables dashboard for the Receivable / Payable tabs
		if (view === 'receivable' || view === 'payable') {
			this.load_rec_payable_view(view);
		}
	...
	update_view_btn_state() {
		this.page.main.find('.fa-tab-bar .fa-tab').removeClass('fa-tab-active');
		this.page.main.find(`.fa-tab-bar .fa-tab[data-tab="${this.current_view}"]`).addClass('fa-tab-active');
	}

	// ✅ Load approvals.js content directly (no iframe, no page reload)
	load_approvals_iframe(view) {
		const me = this;

		// Create the content wrapper once
		if (!this._approvals_content) {
			this._approvals_content = $('<div class="approvals-embed"></div>');
			this._approvals_initialized = false;

			// Inject CSS to hide page chrome within the embed wrapper
			if (!$('#approvals-embed-css').length) {
				$('<style id="approvals-embed-css">').text(`
					.approvals-embed .page-head,
					.approvals-embed .navbar,
					.approvals-embed .layout-side-section,
					.approvals-embed .desk-sidebar,
					.approvals-embed .sidebar {
						display: none !important;
					}
					.approvals-embed .layout-main-section {
						width: 100% !important;
						padding: 0 !important;
						margin: 0 !important;
					}
					.approvals-embed .container,
					.approvals-embed .layout-main {
						width: 100% !important;
						max-width: 100% !important;
						padding: 0 !important;
						margin: 0 !important;
					}
					.approvals-embed .main-tab-btn {
						display: none !important;
					}
					.approvals-embed .approvals-container {
						padding-left: 10px !important;
						padding-right: 10px !important;
					}
					/* Remove the card container border — cards appear directly on page */
					.approvals-embed #dashboard-cards {
					...
					/* Cards: drop the fixed 200px width so they share the row evenly */
					.approvals-embed #dashboard-cards > div {
						flex: 1 1 0 !important;
						min-width: 0 !important;
						width: auto !important;
					}
					.approvals-embed #dashboard-cards > div > div {
						width: auto !important;
						height: 100% !important;
					}

					/* ── Transaction tab ── */
					/* Remove the bordered container wrapping the transaction cards/content */
					.approvals-embed #txn-section {
						border: none !important;
						background: transparent !important;
						border-radius: 0 !important;
						overflow: visible !important;
					}
					.approvals-embed #txn-section > div:first-child {
						margin: 0 0 10px 0 !important;
					}
					/* Transaction analytics cards: single horizontal row, no wrapping */
					.approvals-embed #txn-cards {
						flex-wrap: nowrap !important;
						display: flex !important;
						gap: 12px !important;
						width: 100% !important;
			...
			// Initialize the approvals page inside our wrapper
			const initContent = function () {
				try {
					// Call the approvals page's on_page_load with our wrapper element
					frappe.pages['approvals'].on_page_load(me._approvals_content[0]);
					me._approvals_initialized = true;

					// Inject override CSS AFTER approvals.js injects its own CSS
					// This restores the compare_dashboard's own page-head that
					// approvals.js globally hides via .page-head.flex { display:none }
					if (!$('#approvals-override-css').length) {
						$('<style id="approvals-override-css">').text(`
							.page-head.flex {
								display: flex !important;
							}
							.approvals-embed .page-head.flex {
								display: none !important;
							}
						`).appendTo('head');
					}

					me._show_approvals_tab(view);
				} catch (e) {
					console.error('[approvals embed] init error:', e);
				}
			};

			// Check if approvals script is already loaded
			if (frappe.pages['approvals'] && frappe.pages['approvals'].on_page_load) {
				initContent();
			} else {
				// Load the approvals page data via Frappe's standard page loader
				frappe.views.pageview.with_page('approvals', function () {
					try {
						// Ensure frappe.pages['approvals'] exists before eval
						// approvals.js does: frappe.pages['approvals'].on_page_load = function...
						// Without this, it throws TypeError (cannot set property on undefined)
						frappe.provide('frappe.pages.approvals');

						// Eval the page script which defines on_page_load
						const pageDoc = locals.Page && locals.Page['approvals'];
						if (pageDoc && (pageDoc.__script || pageDoc.script)) {
							frappe.dom.eval(pageDoc.__script || pageDoc.script);
						}
						initContent();
					} catch (e) {
						console.error('[approvals embed] script eval error:', e);
					}
				});
			}
		}

		// Move content to the appropriate mount point
		const mountId = view === 'approvals' ? '#approvals-mount' : '#transaction-mount';
		this.page.main.find(mountId).empty().append(this._approvals_content);

		// Show the correct tab if already initialized
		if (this._approvals_initialized) {
			this._show_approvals_tab(view);
		}
	}

	_show_approvals_tab(view) {
		if (!this._approvals_content) return;
		const $content = this._approvals_content;
		const tab = view === 'approvals' ? 'approvals' : 'transactions';

		// Trigger the main tab click (approvals or transactions)
		$content.find(`.main-tab-btn[data-tab="${tab}"]`).trigger('click');

		// For the Transaction tab, hide all sub-nav buttons except Payment
		const $subnav = $content.find('#approvals-subnav');
		if (view === 'transaction') {
			$subnav.find('.nav-btn').hide();
			$subnav.find('.payment-btn').show();
		} else {
			$subnav.find('.nav-btn').show();
			$subnav.find('.payment-btn').show();
		}
	}

	// ✅ Load the Receivables & Payables dashboard (Custom HTML Block "Payables Outstanding")
	// Reuses the existing page logic — no code duplication. The custom block has its own
	// internal Receivable/Payable tab system; we hide its tab bar and drive it from
	// compare_dashboard's tabs instead.
	load_rec_payable_view(view) {
		const me = this;

		// Create the content wrapper once
		if (!this._recpay_content) {
			this._recpay_content = $('<div class="recpay-embed"></div>');
			this._recpay_initialized = false;

			// Inject CSS to hide the custom block's internal tab bar (compare_dashboard
			// already provides Receivable/Payable tabs) and adjust layout
			if (!$('#recpay-embed-css').length) {
				$('<style id="recpay-embed-css">').text(`
					.recpay-embed .pd-tabs { display: none !important; }
					.recpay-embed #pd-root { padding: 10px 0; }
				`).appendTo('head');
			}

			// Fetch the "Payables Outstanding" Custom HTML Block from the database
			frappe.call({
				method: 'frappe.client.get',
				args: { doctype: 'Custom HTML Block', name: 'Payables Outstanding' },
				callback: function (r) {
					if (!r || !r.message) {
						me._recpay_content.html(
							'<div style="padding:20px;text-align:center;color:#888;">' +
							(me.lang === 'ar' ? 'لم يتم العثور على لوحة البيانات' : 'Dashboard block not found.') +
							'</div>'
						);
						me._recpay_initialized = true;
						me._show_recpay_tab(view);
						return;
					}
					const block = r.message;

					// Inject the block's CSS once
					if (!$('#recpay-block-style').length && block.style) {
						$('<style id="recpay-block-style">').text(block.style).appendTo('head');
					}

					// Inject the block's HTML into our wrapper (provides #pd-root)
					me._recpay_content.html(block.html || '<div id="pd-root"></div>');

					// Eval the block's script with root_element pointing to our wrapper.
					// The script checks `typeof root_element !== 'undefined'` and uses it
					// as the host element, building the tabbed shell inside #pd-root.
					try {
						const root_element = me._recpay_content[0];
						(new Function('root_element', block.script))(root_element);
						me._recpay_initialized = true;
					} catch (e) {
						console.error('[recpay embed] script eval error:', e);
						me._recpay_content.html(
							'<div style="padding:20px;color:#c00;">' +
							(me.lang === 'ar' ? 'حدث خطأ أثناء تحميل لوحة البيانات' : 'Error loading dashboard.') +
							'</div>'
						);
						me._recpay_initialized = true;
					}
		...
		// Move content to the appropriate mount point
		const mountId = view === 'receivable' ? '#receivable-mount' : '#payable-mount';
		this.page.main.find(mountId).empty().append(this._recpay_content);

		// Activate the correct internal tab if already initialized
		if (this._recpay_initialized) {
			this._show_recpay_tab(view);
		}
	}

	_show_recpay_tab(view) {
		if (!this._recpay_content) return;
		const $content = this._recpay_content;
		const tab = view === 'receivable' ? 'receivable' : 'payable';

		// Trigger the custom block's internal tab activation (native click so
		// addEventListener handlers fire). This shows only the relevant section.
		const $tabBtn = $content.find(`.pd-tab[data-tab="${tab}"]`);
		if ($tabBtn.length) {
			$tabBtn[0].click();
		}
	}

	// ✅ Mount the finance-new dashboard inside #finance-new-mount
	load_data_view() {
		...
		$mount.html(`
			...
			<br>
			<div style="background-color:#f5f5f5;display:flex;gap:5px;padding-bottom:10px;margin-left:0px;margin-right:0px;border:1px solid #ddd;border-radius:8px;">
				<div id="tfp-receivable-table1" style="width:100%;border:1px solid #ddd;border-radius:8px;padding:10px;box-sizing:border-box;margin:15px;position:relative;">
					<div style="display:flex;align-items:center;justify-content:space-between;height:50px;background:white;position:sticky;top:0;z-index:1;padding:2px 10px;margin-bottom:-15px;">
						...
						<div style="display:flex;align-items:center;gap:10px;">
							<div id="to_service_filter_dv" style="margin:0;padding:0;"></div>
							<div id="to_am_filter_dv" style="margin:0;padding:0;"></div>
							<div id="to_pm_filter_dv" style="margin:0;padding:0;"></div>
							<button id="download-to-dashboard-dv" class="btn btn-secondary">Download</button>
						</div>
					</div>
					<div id="to-table-content1-dv" style="width:100%;margin-top:15px;"></div>
				</div>
			</div>
			<br>
			<div style="background-color:#f5f5f5;display:flex;gap:5px;padding-bottom:10px;margin-left:0px;margin-right:0px;border:1px solid #ddd;border-radius:8px;">
				<div id="tfp-receivable-table2" style="width:98%;border:1px solid #ddd;border-radius:8px;padding:10px;box-sizing:border-box;margin:15px;position:relative;">
					<div style="display:flex;align-items:center;justify-content:space-between;height:50px;background:white;position:sticky;top:0;z-index:1;padding:2px 10px;margin-bottom:-15px;">
						...
						<div style="display:flex;align-items:center;gap:10px;">
							<div id="ob_service_filter_dv" style="margin:0;padding:0;"></div>
							<div id="ob_am_filter_dv" style="margin:0;padding:0;"></div>
							<div id="ob_pm_filter_dv" style="margin:0;padding:0;"></div>
							<button id="download-ob-dv" class="btn btn-secondary">Download</button>
						</div>
					</div>
					<div id="ob-table-dv" style="width:100%;margin-top:15px;"></div>
				</div>
			</div>
			<br>
			<div style="background-color:#f5f5f5;display:flex;gap:5px;padding-bottom:10px;margin-left:0px;margin-right:0px;border:1px solid #ddd;border-radius:8px;">
				<div id="tfp-receivable-table3" style="width:100%;border:1px solid #ddd;border-radius:8px;padding:10px;box-sizing:border-box;margin:15px;position:relative;">
					<div style="display:flex;align-items:center;justify-content:space-between;height:50px;background:white;position:sticky;top:0;z-index:1;padding:2px 10px;margin-bottom:-15px;">
						...
						<div style="display:flex;align-items:center;gap:10px;">
							<div id="rec_service_filter_dv" style="margin:0;padding:0;"></div>
							<div id="rec_am_filter_dv" style="margin:0;padding:0;"></div>
							<div id="rec_pm_filter_dv" style="margin:0;padding:0;"></div>
							<button id="download9-dashboard-dv" class="btn btn-secondary">Download</button>
						</div>
					</div>
					<div id="receivable-so-table-content1-dv" style="width:100%;margin-top:15px;"></div>
				</div>
			</div>
			<br>
			<div style="background-color:#f5f5f5;display:flex;gap:5px;margin-top:5px;padding-bottom:10px;margin-left:0px;margin-right:0px;border:1px solid #ddd;border-radius:8px;">
				<div id="tfp-receivable-table4" style="width:100%;width:98%;border:1px solid #ddd;border-radius:8px;padding:10px;box-sizing:border-box;margin:15px;position:relative;">
					<div style="display:flex;align-items:center;justify-content:space-between;height:50px;background:white;position:sticky;top:0;z-index:1;padding:2px 10px;margin-bottom:-15px;">
						...
						<div style="display:flex;align-items:center;gap:10px;">
							<div id="bill_service_filter_dv" style="margin:0;padding:0;"></div>
							<div id="bill_am_filter_dv" style="margin:0;padding:0;"></div>
							<div id="bill_pm_filter_dv" style="margin:0;padding:0;"></div>
							<button id="download10-dashboard-dv" class="btn btn-secondary">Download</button>
						</div>
					</div>
					<div id="tobill-so-table-content1-dv" style="width:100%;margin-top:15px;"></div>
				</div>
			</div>
			<br>
			<div style="background-color:#f5f5f5;display:flex;gap:5px;margin-top:5px;padding-bottom:10px;margin-left:0px;margin-right:0px;border:1px solid #ddd;border-radius:8px;">
				<div id="tfp-receivable-table5" style="width:100%;width:98%;border:1px solid #ddd;border-radius:8px;padding:10px;box-sizing:border-box;margin:15px;position:relative;">
					<div style="display:flex;align-items:center;justify-content:space-between;height:50px;background:white;position:sticky;top:0;z-index:1;padding:2px 10px;margin-bottom:-15px;">
						...
						<div style="display:flex;align-items:center;gap:10px;">
							<div id="pay_service_filter_dv" style="margin:0;padding:0;"></div>
							<div id="pay_am_filter_dv" style="margin:0;padding:0;"></div>
							<div id="pay_pm_filter_dv" style="margin:0;padding:0;"></div>
							<button id="download11-dashboard-dv" class="btn btn-secondary">Download</button>
						</div>
					</div>
					<div id="payable-so-table-content-dv" style="width:100%;margin-top:15px;"></div>
				</div>
			</div>
			<br>
			<div style="background-color:#f5f5f5;display:flex;gap:5px;margin-top:5px;padding-bottom:10px;margin-left:0px;margin-right:0px;border:1px solid #ddd;border-radius:8px;">
				<div id="tfp-receivable-table6" style="width:100%;width:98%;border:1px solid #ddd;border-radius:8px;padding:10px;box-sizing:border-box;margin:15px;position:relative;">
					<div style="display:flex;align-items:center;justify-content:space-between;height:50px;background:white;position:sticky;top:0;z-index:1;padding:2px 10px;margin-bottom:-15px;">
						...
						<div style="display:flex;align-items:center;gap:10px;">
							<div id="tb_service_filter_dv" style="margin:0;padding:0;"></div>
							<div id="tb_am_filter_dv" style="margin:0;padding:0;"></div>
							<div id="tb_pm_filter_dv" style="margin:0;padding:0;"></div>
							<button id="download-tb-dv" class="btn btn-secondary">Download</button>
						</div>
					</div>
					<div id="tb-table-dv" style="width:100%;margin-top:15px;"></div>
				</div>
			</div>
			...
			<div style="background-color:#f5f5f5; display:flex; gap:20px; margin:30px 15px; padding:10px; border:1px solid #ddd; border-radius:8px;">
				<div style="flex:1; border:1px solid #ddd; border-radius:8px; padding:10px; box-sizing:border-box; display:flex; flex-direction:column; min-width:0;">
					<h4 style="margin:10px; text-align:center; background:white; position:sticky; top:0; z-index:2;">VENDING MACHINE SALES</h4>
					<div style="max-height:400px; overflow:auto;"><div id="vm-sales-dv"></div></div>
				</div>
				<div style="flex:1; border:1px solid #ddd; border-radius:8px; padding:10px; box-sizing:border-box; display:flex; flex-direction:column; min-width:0;">
					<h4 style="margin:10px; text-align:center; background:white; position:sticky; top:0; z-index:2;">RETAIL SHOP SALES</h4>
					<div style="max-height:400px; overflow:auto;"><div id="retail-sales-dv"></div></div>
				</div>
			</div>

			<div style="margin:30px 15px; border:1px solid #ddd; border-radius:8px; padding:10px; background-color:#f5f5f5;">
				<div style="display:flex; justify-content:space-between; align-items:center; background:white; border-radius:5px; padding:12px 15px; margin-bottom:10px;">
					<h3 style="margin:0; font-size:18px; font-weight:700; color:#000;">TARGET VS ACHIEVEMENT REPORT</h3>
					<div style="display:flex; align-items:center; gap:10px;">
						<div id="tb_service_filter_target_dv" style="min-width:180px;"></div>
						<div id="tb_employee_filter_dv" style="min-width:180px;"></div>
						<div id="tb_target_filter_dv" style="min-width:180px;"></div>
						<button id="download_target_achievement_dv" class="btn btn-secondary">Download</button>
					</div>
				</div>
				<div style="overflow:auto; max-height:400px;">
					<table class="table table-bordered" style="margin-bottom:0;">
						...
						<tbody id="target-vs-achievement-body-dv"></tbody>
					</table>
				</div>
			</div>
		`);
		...
		$(`<style>
			...
			.data-view-wrapper .dashboard-card,
			.data-view-wrapper .po_out,
			.data-view-wrapper .po,
			.data-view-wrapper .amount{
				min-width:240px; min-height:170px; height:auto; border-radius:20px; background:#fff;
				border-top:5px solid #f59e0b; box-shadow:0 4px 14px rgba(0,0,0,.08);
				position:relative; overflow:hidden; padding:12px;
			}
			.data-view-wrapper .dashboard-card::before,
			.data-view-wrapper .po_out::before,
			.data-view-wrapper .po::before,
			.data-view-wrapper .amount::before{
				content:''; position:absolute; top:-25px; right:-25px; width:80px; height:80px; border-radius:50%;
			}
			.data-view-wrapper .turnover-card1{border-top-color:#4f46e5;}
			.data-view-wrapper .turnover-card1::before{background:#ede9fe;}
			.data-view-wrapper .order-booking-card1{border-top-color:#10b981;}
			.data-view-wrapper .order-booking-card1::before{background:#d1fae5;}
			.data-view-wrapper .order-booking-card{border-top-color:#3b82f6;}
			.data-view-wrapper .order-booking-card::before{background:#dbeafe;}
			.data-view-wrapper .turnover-card{border-top-color:#ec4899;}
			.data-view-wrapper .turnover-card::before{background:#fce7f3;}
			.data-view-wrapper .po_out{border-top-color:#14b8a6;}
			.data-view-wrapper .po_out::before{background:#ccfbf1;}
			.data-view-wrapper .po{border-top-color:#f97316;}
			.data-view-wrapper .po::before{background:#ffedd5;}
			.data-view-wrapper .amount{border-top-color:#06b6d4;}
			.data-view-wrapper .amount::before{background:#cffafe;}
			.data-view-wrapper .card-title{text-align:center; font-size:16px; font-weight:bold; margin-bottom:10px;}
			.data-view-wrapper .card-total{text-align:center; margin-bottom:15px;}
			.data-view-wrapper .card-total span{display:inline-block; background:#e8f5e9; padding:4px 14px; border-radius:50px; color:green; font-weight:bold; font-size:22px;}
			.data-view-wrapper .service-grid{border-top:1px solid #eee; padding-top:8px; display:grid; grid-template-columns:repeat(3,1fr); column-gap:0px; row-gap:6px; width:100%;}
			.data-view-wrapper .service-row{display:flex; align-items:center; justify-content:flex-start; min-width:0; white-space:nowrap;}
			.data-view-wrapper .service-name{font-size:10px; font-weight:bold; color:black; margin-right:1px;}
			.data-view-wrapper .service-value{font-size:10px; color:red; font-weight:bold;}

			/* ✅ Filter alignment fix for the 6 tables */
			.data-view-wrapper [id$="_filter_dv"]{
				display:flex;
				align-items:center;
			}
			.data-view-wrapper .frappe-control{
				margin-bottom:0 !important;
				padding-bottom:0 !important;
			}
			.data-view-wrapper .form-group{
				margin-bottom:0 !important;
				display:flex;
				align-items:center;
			}
			.data-view-wrapper .form-group .control-input-wrapper,
			.data-view-wrapper .form-group .control-input{
				display:flex;
				align-items:center;
			}
			.data-view-wrapper .frappe-control input{
		...
		this.init_finance_new_logic();
	}

	// ✅ All the finance-new.js JS logic, scoped to the data-view-wrapper, suffixed with _dv
	init_finance_new_logic() {
			...
			const rows = order
				...
				.map(key => `
					<div class="service-row">
						...
						<span class="service-value">${formatToLakhs(groups[key])}</span>
					</div>
				`);
			return rows.join('');
		}
		...
		function renderCard(selector, title, total, groups, order) {
			...
			$scope.find(selector).html(`
				...
				<div class="card-total"><span>${formatToLakhs(total)}</span></div>
				<div class="service-grid">${gridHtml}</div>
			`);
		}

		function add_filter_dv(parent_id, df, callback_function = null) {
			$scope.find(parent_id).empty();
			const control = frappe.ui.form.make_control({
				parent: $scope.find(parent_id),
				df: Object.assign({
					reqd: 0,
					onchange: function () {
		...
		// Filters
		let rec_service_filter = add_filter_dv('#rec_service_filter_dv', { fieldtype: 'Link', options: 'Services', fieldname: 'rec_service', placeholder: 'Service' }, load_receivable_overall_table);
		let rec_am_filter = add_filter_dv('#rec_am_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'rec_am', placeholder: 'AM' }, load_receivable_overall_table);
		let rec_pm_filter = add_filter_dv('#rec_pm_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'rec_pm', placeholder: 'PM' }, load_receivable_overall_table);

		let to_service_filter = add_filter_dv('#to_service_filter_dv', { fieldtype: 'Link', options: 'Services', fieldname: 'to_service', placeholder: 'Service' }, load_turnover_overall_table);
		let to_am_filter = add_filter_dv('#to_am_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'to_am', placeholder: 'AM' }, load_turnover_overall_table);
		let to_pm_filter = add_filter_dv('#to_pm_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'to_pm', placeholder: 'PM' }, load_turnover_overall_table);

		let bill_service_filter = add_filter_dv('#bill_service_filter_dv', { fieldtype: 'Link', options: 'Services', fieldname: 'bill_service', placeholder: 'Service' }, load_tobill_overall_table);
		let bill_am_filter = add_filter_dv('#bill_am_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'bill_am', placeholder: 'AM' }, load_tobill_overall_table);
		let bill_pm_filter = add_filter_dv('#bill_pm_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'bill_pm', placeholder: 'PM' }, load_tobill_overall_table);

		let ob_service_filter = add_filter_dv('#ob_service_filter_dv', { fieldtype: 'Link', options: 'Services', fieldname: 'ob_service', placeholder: 'Service' }, load_ob_overall_table);
		let ob_am_filter = add_filter_dv('#ob_am_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'ob_am', placeholder: 'AM' }, load_ob_overall_table);
		let ob_pm_filter = add_filter_dv('#ob_pm_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'ob_pm', placeholder: 'PM' }, load_ob_overall_table);

		let tb_service_filter = add_filter_dv('#tb_service_filter_dv', { fieldtype: 'Link', options: 'Services', fieldname: 'tb_service', placeholder: 'Service' }, load_to_book_table);
		let tb_am_filter = add_filter_dv('#tb_am_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'tb_am', placeholder: 'AM' }, load_to_book_table);
		let tb_pm_filter = add_filter_dv('#tb_pm_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'tb_pm', placeholder: 'PM' }, load_to_book_table);
		$scope.find('#tb_am_filter_dv').hide();
		$scope.find('#tb_pm_filter_dv').hide();

		let pay_service_filter = add_filter_dv('#pay_service_filter_dv', { fieldtype: 'Link', options: 'Services', fieldname: 'pay_service', placeholder: 'Service' }, load_payable_table);
		let pay_am_filter = add_filter_dv('#pay_am_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'payc_am', placeholder: 'AM' }, load_payable_table);
		let pay_pm_filter = add_filter_dv('#pay_pm_filter_dv', { fieldtype: 'Link', options: 'User', fieldname: 'pay_pm', placeholder: 'PM' }, load_payable_table);
		$scope.find('#pay_am_filter_dv').hide();
		$scope.find('#pay_pm_filter_dv').hide();
		...
		// Downloads
		$scope.on('click', '#download-to-dashboard-dv', function () {
			const path = "teampro.teampro.page.finance_details.tfp_dashboard.download_to_table_overall_excel";
			window.location.href = frappe.request.url + '?cmd=' + path
				+ '&overall_service=' + encodeURIComponent(to_service_filter.get_value() || "")
				+ '&account_manager=' + encodeURIComponent(to_am_filter.get_value() || "")
				+ '&project_manager=' + encodeURIComponent(to_pm_filter.get_value() || "");
		});

		$scope.on('click', '#download-ob-dv', function () {
			const path = "teampro.teampro.page.finance_details.tfp_dashboard.download_ob_excel";
			window.location.href = frappe.request.url + '?cmd=' + path
				+ '&overall_service=' + encodeURIComponent(ob_service_filter.get_value() || "")
				+ '&account_manager=' + encodeURIComponent(ob_am_filter.get_value() || "")
				+ '&project_manager=' + encodeURIComponent(ob_pm_filter.get_value() || "");
		});

		$scope.on('click', '#download9-dashboard-dv', function () {
			const path = "teampro.teampro.page.finance_details.tfp_dashboard.download_receivable_table_overall";
			window.location.href = frappe.request.url + '?cmd=' + path
				+ '&service=' + encodeURIComponent(rec_service_filter.get_value() || "")
				+ '&am=' + encodeURIComponent(rec_am_filter.get_value() || "")
				+ '&pm=' + encodeURIComponent(rec_pm_filter.get_value() || "");
		});

		$scope.on('click', '#download10-dashboard-dv', function () {
			const path = "teampro.teampro.page.finance_details.tfp_dashboard.download_tobill_excel";
			window.location.href = frappe.request.url + '?cmd=' + path
				+ '&overall_service=' + encodeURIComponent(bill_service_filter.get_value() || "")
				+ '&account_manager=' + encodeURIComponent(bill_am_filter.get_value() || "")
				+ '&project_manager=' + encodeURIComponent(bill_pm_filter.get_value() || "");
		});

		$scope.on('click', '#download11-dashboard-dv', function () {
			const path = "teampro.teampro.page.finance_details.tfp_dashboard.download_payable_table1";
			window.location.href = frappe.request.url + '?cmd=' + path
				+ '&overall_service=' + encodeURIComponent(pay_service_filter.get_value() || "")
				+ '&account_manager=' + encodeURIComponent(pay_am_filter.get_value() || "")
				+ '&project_manager=' + encodeURIComponent(pay_pm_filter.get_value() || "");
		});

		$scope.on('click', '#download_target_achievement_dv', function () {
			window.location.href = "/api/method/teampro.teampro.page.finance.finance_dashboard.download_target_achievement";
		});

		// Card click → scroll to table
		function bindCardScroll(cardClass, tableId) {
			$(document).off("click", "." + cardClass).on("click", "." + cardClass, function (e) {
				if (!$scope.is(':visible')) return; // only when data view active
				e.preventDefault();
				const targetElement = $scope.find("#" + tableId)[0];
				if (targetElement) {
		...
		bindCardScroll('po_out', 'tfp-receivable-table5');
		bindCardScroll('turnover-card1', 'tfp-receivable-table1');
		bindCardScroll('order-booking-card1', 'tfp-receivable-table2');
		bindCardScroll('order-booking-card', 'tfp-receivable-table3');
		bindCardScroll('turnover-card', 'tfp-receivable-table4');
		bindCardScroll('po', 'tfp-receivable-table6');

		// VM Sales / Retail Sales / Target vs Achievement
		frappe.call({
			method: "teampro.teampro.page.finance.finance_dashboard.vm_sales",
			callback: function (r) {
				$scope.find('#vm-sales-dv').html(r.message || `<div style="padding:10px;text-align:center">No data found</div>`);
			}
		});
		...
		frappe.call({
			...
			callback: function (r) {
				$scope.find('#retail-sales-dv').html(r.message || `<div style="padding:10px;text-align:center">No data found</div>`);
			}
		});
	...
	render_all() {
		...
		this.render_performance_summary_chart();
	...
	render_performance_summary_chart() {
		...
		$.ajax({
			...
			success: (r) => {
				if (r && r.message) {
					this.render_performance_summary_echart(r.message);
				}
			},
			error: () => {
				this.$performance_summary_chart.html(this.empty_msg());
			}
		});
	}

	render_performance_summary_echart(data) {
		if (!window.echarts) {
			setTimeout(() => this.render_performance_summary_echart(data), 500);
			return;
		}
		if (!data.turnover.length && !data.order_booking.length && !data.payables.length) {
			this.$performance_summary_chart.html(this.empty_msg());
			this.$performance_summary_stats.empty();
			return;
		}

		this.$performance_summary_chart.empty();

		const months = new Set();
		data.turnover.forEach(d => months.add(d.month));
		data.order_booking.forEach(d => months.add(d.month));
		data.payables.forEach(d => months.add(d.month));
		const sorted_months = Array.from(months).sort();

		const turnover_map = {};
		data.turnover.forEach(d => turnover_map[d.month] = d.total || 0);

		const order_booking_map = {};
		data.order_booking.forEach(d => order_booking_map[d.month] = d.total || 0);

		const payables_map = {};
		data.payables.forEach(d => payables_map[d.month] = d.total || 0);

		const turnover_values = sorted_months.map(m => turnover_map[m] || 0);
		const order_booking_values = sorted_months.map(m => order_booking_map[m] || 0);
		const payables_values = sorted_months.map(m => payables_map[m] || 0);

		const labels = sorted_months.map(m => {
			const [year, month] = m.split('-');
			return `${this.month_name(parseInt(month))} ${year}`;
		});

		const total_turnover = turnover_values.reduce((a, b) => a + b, 0);
		const total_order_booking = order_booking_values.reduce((a, b) => a + b, 0);
		const total_payables = payables_values.reduce((a, b) => a + b, 0);

		this.$performance_summary_stats.html(`
			<div class="chart-stat"><div class="chart-stat-label">${this.t('chart_turnover')}</div><div class="chart-stat-value positive">${this.fc(total_turnover)}</div></div>
			<div class="chart-stat"><div class="chart-stat-label">${this.t('chart_order_booking')}</div><div class="chart-stat-value" style="color:#4361ee">${this.fc(total_order_booking)}</div></div>
			<div class="chart-stat"><div class="chart-stat-label">${this.t('chart_payables')}</div><div class="chart-stat-value" style="color:#f97316">${this.fc(total_payables)}</div></div>
		`);

		this.init_echart(this.$performance_summary_chart[0], {
			tooltip: {
				trigger: 'axis',
			...
			legend: {
				data: [this.t('chart_turnover'), this.t('chart_order_booking'), this.t('chart_payables')],
				bottom: 0,
			...
			yAxis: {
				type: 'value',
				axisLabel: { formatter: (v) => this.short_number(v), fontFamily: 'Cairo', fontSize: 11 }
			},
			series: [
				{
					name: this.t('chart_turnover'),
					type: 'bar',
					data: turnover_values,
					itemStyle: { color: '#10b981', borderRadius: [4, 4, 0, 0] },
					barMaxWidth: 28
				},
				{
					name: this.t('chart_order_booking'),
					type: 'bar',
					data: order_booking_values,
					itemStyle: { color: '#4361ee', borderRadius: [4, 4, 0, 0] },
					barMaxWidth: 28
				},
				{
					name: this.t('chart_payables'),
					type: 'bar',
					data: payables_values,
