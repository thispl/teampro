...
from teampro.teampro.doctype.daily_monitor.dm_it_dev import dpr_task_mail_it_dev
from frappe.utils import today

class DailyMonitor(Document):
    
    def validate(self):
        if self.service == "IT-SW":
            for row in self.task_details:
                if row.id and 'ISS-' in row.id:
                    status =frappe.db.get_value('Issue',{'name':row.id},['status'])
                    row.project_name=frappe.db.get_value('Issue',{'name':row.id},['project'])
                    row.subject=frappe.db.get_value('Issue',{'name':row.id},['subject'])
                    row.current_status=frappe.db.get_value('Issue',{'name':row.id},['custom_issue_status'])
                    row.priority=frappe.db.get_value('Issue',{'name':row.id},['priority'])
                    row.allocated_to=frappe.db.get_value('Issue',{'name':row.id},['assigned_to'])
                else:
                    status =frappe.db.get_value('Task',{'name':row.id},['status'])
                    row.allocated_to=frappe.db.get_value('Task',{'name':row.id},['custom_allocated_to'])
                    row.cb_against_task = frappe.db.get_value("Task",{'name':row.id},['cb'])
                if status:
            ...
            for row in self.task_details:
                ...
                if row.issue_cb:
                    cb_rt_map.setdefault(row.issue_cb, 0)
                    cb_rt_map[row.issue_cb] += row.issue_rt or 0
...
def update_rec_dpr(date,name,service):
    date_obj = datetime.strptime(date, '%Y-%m-%d')
    formatted_date = date_obj.strftime('%d/%m/%Y')
    emp=frappe.db.get_all("Employee",{'status':'Active','reports_to':'TI00003','user_id':('not in',['keerthana.k@groupteampro.com','narendharan.k@groupteampro.com'])},['*'])
    recievers=[]
    for i in emp:
        recievers.append(i.user_id)
