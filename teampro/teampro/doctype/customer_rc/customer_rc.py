...
class CustomerRC(Document):
	...
	def validate(self):
		existing = frappe.db.exists("Customer RC",{"customer_name": self.customer_name, "name": ["!=", self.name]})
		if existing:
	...
	def on_submit(self):
		...
		if not frappe.db.exists("Customer", {"customer_name": self.customer_name}):
			...
			customer = frappe.get_doc({
				...
				"account_manager":"dineshbabu.k@groupteampro.com",
